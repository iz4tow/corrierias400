...
###############################################################TASTO INSERISCI
	if button == "Inserisci":
		...
		curs.execute("SELECT * FROM CTEMELTST.TXVET20F where VPCLIF='"+codcli+"' AND VPPMAX='"+peso+"'")
		...
		if esiste>0 or esiste_cli>0:#SE ESISTE IL CLIENTE+PESO O IL CORRIERE IN ANAGRAFICA
			...
			if errore==0:
				try:#INSERIMENTO IN TRTTRA00F
					...
					app.setLabelFg("avviso1", "red")
					print("INSERT INTO CTEMELTST.TRTRA00F (TRATR0,TRPFU0,TRDTC0,TRDTA0,TRCCOM,TRCDTB,TRCDEC,TRCDEL) VALUES (' ','"+utente+"','"+data+"','000000','"+committente+"','VET','"+codvettore+"','"+elemento+"')")#DEBUG CON CONSOLE ATTIVA
			if errore==0:
				try:#INSERIMENTO IN BATRA00F
					...
					app.setLabelFg("avviso1", "red")
					print("INSERT INTO CTEMELTST.BATRA00F (BTATRC,BTCTRA,BTRAGS,BTRAGA,BTINDI,BTLOCA,BTPROV,BTCCAP,BTCNAZ,BTCFIS,BTPIVA,BTCBAN,BTCAGE,BTDAGE,BTCVAL,BTPFUT,BTDCRE,BTDAGG) VALUES ('','"+codvettore+"','"+ragsocvettore+"','','"+indirizzo+"','"+localita+"','"+provincia+"','"+cap+"','','.','','','','','','"+utente+"','20"+data+"','00000000')")#DEBUG CON CONSOLE ATTIVA
			if errore==0:
			...
			if errore==0:
				try:#INSERIMENTO IN TXVET20F
					...
					app.setLabelFg("avviso1", "red")
					print("INSERT INTO CTEMELTST.TXVET20F (VPATRC,VPCCOM,VPCVET,VPPMAX,VPPFUT,VPDCRE,VPDAGG,VPCMAG,VPCLIF) VALUES ('','"+committente+"','"+elemento+"','"+peso+"','"+utente+"','20"+data+"','00000000','"+magazzino+"','"+codcli+"')")#DEBUG CON CONSOLE ATTIVA
...
###############################################################TASTO RICERCA		
	...
	if button == "Ricerca":
		...
		if len(codvettore)>0:#CONTROLLO CHE SIA STATO INSERITO UN CODICE VETTORE
			curs.execute("SELECT TRCDEL FROM CTEMELTST.TRTRA00F where TRCDEC='"+codvettore+"'") #CERCO IL CODICE S72 SU TRTRA00F
			rows=curs.fetchall() 
			esiste=len(rows)
		if esiste>0:#SE ESISTE IL CORRIERE ESTRAGGO L'ANAGRAFICA ASSOCIATA
			elemento=str(rows[0][0])
			curs.execute("SELECT BTRAGS FROM CTEMELTST.BATRA00F where BTCTRA='"+codvettore+"'") #CERCO IL CODICE S72 SU TRTRA00F
			rows=curs.fetchall() 
			ragsocvettore=str(rows[0][0])
			curs.execute("SELECT VPPMAX,VPCMAG FROM CTEMELTST.TXVET20F where VPCVET='"+elemento+"'") #CERCO IL CODICE S72 SU TRTRA00F
			rows=curs.fetchall() 
			...
			curs.execute("SELECT TBCDTB FROM CTEMELTST.TRTAB00F where TBCDEL='       "+elemento+"'") #CERCO elemento su tabella bartolini
		...
		if len(codcli)>0:#CONTROLLO CHE SIA STATO INSERITO UN CODICE CLIENTE
			curs.execute("SELECT DERAGS,DEINDI,DELOCA,DEPROV,DENAZI,DECAPC FROM CTEMELTST.BADES00F where DECEXT='"+codcli+"'") #CERCO IL CODICE S72 SU TRTRA00F
			rows=curs.fetchall() 
			esiste=len(rows)
